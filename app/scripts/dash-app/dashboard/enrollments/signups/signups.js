angular.module('dashboard.enrollments.signups', [
	...
	.controller('SignupCtrl',  function ($scope, SignupModel) {
		...
		function noData(){
		  	elt.parent().hide();
		  	showGraphArea(false);
		  	elt.closest('.panel-body').find('.panel-body-stat').text('\u2014');
		  	elt.closest('.panel-body').prev().find('.panel-stat').text('\u2014');
	  	}
	})
		...
		return {
			...
			link: function postLink(scope, elem, attrs) {
				...
				function populateGraph(){
					...
					// graph colors are based on the color of the bar at the top of the panel
					...
					// if (newGraph){
					// 	elem.closest('.panel-body').find('.panel-loading').fadeOut();
					// }
				}
