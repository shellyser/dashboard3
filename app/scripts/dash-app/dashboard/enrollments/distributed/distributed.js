angular.module('dashboard.enrollments.distributed', [
	...
	.controller('DistributedCtrl',  function ($scope, DistributedModel) {
		...
		$scope.setDelivery = function(action) {
		  $scope.distributedparams.deliverySelected = action;
		};
	    ...
	    link: function postLink(scope, elem, attrs) {  
	      ...
	      function populateGraph(){
	        ...
	        for (var pointSet in points){ 
	          var checkbox;
	          if (points.length == 1){
	            if (elem.closest('.panel-body').find('.module-view-options-checkbox label:eq(0) input').attr("checked") === "checked"){
	              checkbox = 0;
	            }
	            else{
	              checkbox = 1;
